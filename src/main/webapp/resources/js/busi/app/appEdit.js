...
app.import = {
    init: function () {
        $("#uploadFile").uploadifive({
            ...
            multi: false,
            queueID: 'fileQueue',
           /* fileType: 'application/vnd.ms-apk,text/plain',*/
            buttonText: "选择文件",
            formData: "",
            'uploadScript': ctx + '/fileUpload/uploadApk',
            'onUploadComplete': function (file, data) {
                var json = eval('(' + data + ')');
                if (json.status == 'OK') {
                    $("#appLocation").val(json.message);
                } else {
        ...
        $("#uploadFileImg").uploadifive({
            auto: true,
            multi: false,
            queueID: 'fileQueueImg',
           /* fileType: 'application/vnd.ms-png,text/plain',*/
            buttonText: "选择文件",
            formData: "",
            'uploadScript': ctx + '/fileUpload/uploadImg',
            'onUploadComplete': function (file, data) {
                var json = eval('(' + data + ')');
                if (json.status == 'OK') {
                    $("#iconLocation").val(json.message);
                } else {
                    $.pnotify({title: json.message, type: json.status});
                }
            }
      ...
      /*      rules: {
            ...
            submitHandler: function (form) {
                if( $("#appLocation").val() == null || $("#appLocation").val() == ""){
                    $("#uploadErrorMsg").html("请上传应用文件");
                }
                else if( $("#iconLocation").val() == null || $("#iconLocation").val() == ""){
                    $("#uploadErrorMsgImg").html("请上传图标文件");
                }
                else {

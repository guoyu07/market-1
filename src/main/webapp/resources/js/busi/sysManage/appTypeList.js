...
user.list = {
    ...
    getRecords: function () {
        ucf.ajax.commonFn("type/list", $("#frmQry").serialize(), function (result) {
                ...
                if (typeof result.message.content != "undefined")
                    $.each(result.message.content, function (index, obj) {
                        html += '<tr id="areaInfo_' + obj.id + '">'
                            ...
                            + '<td>' + obj.remark + '</td>'
                            + '<td><a href="area/del?id='+obj.id+'">删除</a>  <a href="/app/type/toAdd?id='+obj.id+'&name='+ obj.name +'&remark='+obj.remark+'" data-toggle="modal" data-target="#myModal">编辑</a></td>'
                            + '</tr>';
                    });
